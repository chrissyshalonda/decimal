...
void s21_from_big_decimal_to_string(s21_big_decimal value, char *decimal) {
  char *binary = malloc(256);  // Увеличиваем размер для big_decimal
  int sign = s21_from_big_decimal_to_binary_string(value, binary);
...
int s21_from_big_decimal_to_binary_string(s21_big_decimal value, char *binary) {
  // Переводим big_decimal в строку единиц и нулей

  if (s21_big_get_sign(value)) {
    s21_big_negate(value, &value);
    s21_big_set_sign(&value, 1);
  }

  char *ptr = binary;
  for (int i = 0; i < 7; i++) {  // Увеличиваем до 7 для big_decimal
    for (int j = 0; j < 32; j++) {
      *ptr++ = (value.bits[i] & (1u << j)) ? '1' : '0';
    }
  }
