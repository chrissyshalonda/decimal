...
void s21_big_pretty_print_bits(s21_big_decimal value) {
    ...
    for(int i = 7; i >= 0; i--){
        ...
        printf("bit[%d]: ", i);
        for(int j = 31; j >= 0; j--){
            if(i == 7 && (j >=0 && j <= 15)) printf(RED);
            else if(i == 7 && (j >=16 && j <= 23)) printf(GREEN);
            else if(i == 7 && (j >=24 && j <= 30)) printf(BLUE);
            else if(i == 7 && j == 31) printf(MAGENTA);
            else printf(RESET);
            printf("%d ", s21_big_get_bit(value, i * 32 + j));
        }
    ...
    printf(BG_RED"    "RESET": Биты от 224 до 239, младшее слово, не используются и должны быть равны нулю.\n");
    printf(BG_GREEN"    "RESET": Биты с 240 по 247 должны содержать показатель степени от 0 до 68.\n");
    printf(BG_BLUE"    "RESET": Биты с 248 по 254 не используются и должны быть равны нулю.\n");
    printf(BG_MAGENTA"    "RESET": Бит 255 содержит знак; 0 означает положительный, а 1 означает отрицательный.\n\n\n");
