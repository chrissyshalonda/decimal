...
#define START_TEST(__testname)                                              \
  ...
  static const TTest __testname##_ttest = {"" #__testname, __testname##_fn, \
                                           __FILE__, __LINE__};             \
 ...
 */
#define ck_abort_msg(...) \
  _ck_assert_failed(__FILE__, __LINE__, "Failed", ##__VA_ARGS__)
...
#define _ck_assert_floating_op_tol(X, OP, Y, T, D, TP, TM)                \
  do {                                                                    \
    ...
    ck_assert_msg((_ck_x - _ck_y) OP _ck_t *(D),                          \
                  ...
                  "g, %s == %.*" TM "g",                                  \
                  #X " " #OP "= " #Y ", error < " #T, #X,                 \
                  (int)CK_FLOATING_DIG, _ck_x, #Y, (int)CK_FLOATING_DIG,  \
                  _ck_y, #T, (int)CK_FLOATING_DIG, _ck_t);                \
  } while (0)
 ...
 * @since 0.11.0
 */
#define ck_assert_ldouble_eq(X, Y) \
 ...
 * @since 0.11.0
 */
#define ck_assert_ldouble_ne(X, Y) \
  _ck_assert_floating(X, !=, Y, long double, "L")
/**
 * Check two double precision floating point numbers to determine if X < Y
 *
 ...
 * @since 0.11.0
 */
#define ck_assert_ldouble_lt(X, Y) \
  _ck_assert_floating(X, <, Y, long double, "L")
/**
 * Check two double precision floating point numbers to determine if X <= Y
 *
 ...
 * @since 0.11.0
 */
#define ck_assert_ldouble_le(X, Y) \
  _ck_assert_floating(X, <=, Y, long double, "L")
/**
 * Check two double precision floating point numbers to determine if X > Y
 *
 ...
 * @since 0.11.0
 */
#define ck_assert_ldouble_gt(X, Y) \
  _ck_assert_floating(X, >, Y, long double, "L")
/**
 * Check two double precision floating point numbers to determine if X >= Y
 *
 ...
 * @since 0.11.0
 */
#define ck_assert_ldouble_ge(X, Y) \
  _ck_assert_floating(X, >=, Y, long double, "L")
 ...
 * @since 0.11.0
 */
#define ck_assert_ldouble_finite(X) \
  _ck_assert_floating_finite(X, long double, "L")
 ...
 * @since 0.11.0
 */
#define ck_assert_ldouble_infinite(X) \
  _ck_assert_floating_infinite(X, long double, "L")
 ...
 * @since 0.11.0
 */
#define ck_assert_ldouble_nonnan(X) \
  _ck_assert_floating_nonnan(X, long double, "L")
 ...
 */
/* String pointer could be compared againts NULL with == (NULLEQ = 1) and !=
 * (NULLNE = 1) operators. */
...
#define _ck_assert_str(X, OP, Y, NULLEQ, NULLNE)                         \
  do {                                                                   \
    ...
    ck_assert_msg((NULLEQ && (_ck_x == NULL) && (_ck_y == NULL)) ||      \
                  ...
                  #X " " #OP " " #Y, #X, _ck_x_q, _ck_x_s, _ck_x_q, #Y,  \
...
/* Memory location comparison macros with improved output compared to
 * ck_assert() */
/* OP might be any operator that can be used in '0 OP memcmp(X,Y,L)' comparison
 */
/* The x and y parameter swap in memcmp() is needed to handle >, >=, <, <=
 * operators */
...
/**
 * Check two memory locations to determine if 0<memcmp(X,Y,L), (e.g.
 * memcmp(X,Y,L)>0)
 *
 * If not 0<memcmp(X,Y,L), the test fails.
 *
...
#define _ck_assert_ptr(X, OP, Y)                                              \
  do {                                                                        \
    ...
    const void *_ck_y = (Y);                                                  \
    ck_assert_msg(_ck_x OP _ck_y,                                             \
                  "Assertion '%s' failed: %s == %#lx, %s == %#lx",            \
                  #X " " #OP " " #Y, #X, (unsigned long)(uintptr_t)_ck_x, #Y, \
...
#define _ck_assert_ptr_null(X, OP)                                          \
  do {                                                                      \
    const void *_ck_x = (X);                                                \
    ck_assert_msg(_ck_x OP NULL, "Assertion '%s' failed: %s == %#lx",       \
                  #X " " #OP " NULL", #X, (unsigned long)(uintptr_t)_ck_x); \
  } while (0)
 ...
 *
 * This protects the code against unintentional extremely large assertion
 * messages (values of up to 4GB were seen in the wild). The usual size for a
 * message is less than 80 bytes.
 *
 * If the environment variable CK_MAX_MSG_SIZE is defined to a positive value,
 * it is used. Otherwise, if a positive maximal message size is set via this
 * function, it is used. Otherwise, the maximal message size is one assigned at
 * compile time (4K bytes).
 *
